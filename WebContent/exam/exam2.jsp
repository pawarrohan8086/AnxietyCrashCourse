...
<body>
	<div>
		...
		<form action="../AddAnswers" name="mcQuestion" id="mcQuestion"
			method="get">
			<div class="d-flex bd-highlight mb-3 header" id="myHeader">
				...
				<div class=" p-2 bd-highlight">
					<strong>Remaining Time : <span id="display"
						style="color: #FF0000; font-size: 20px"></span>
					</strong>
			...
			<div class="content">
				...
				<div class="scp-quizzes-main table-info "
					...
					<div class="scp-quizzes-data" style="margin-top: 20px;">
						...
						<p>
							...
							</strong> <input type="hidden" name="quetext<%=qid%>"
								value="<%=qued.getQuetext()%>"> <input type="hidden"
								name="anstext<%=qid%>" value="<%=qued.getAnstext()%>">
						</p>
						<p>
							<strong
				...
				<div class="panel-footer">
					...
					<input type="radio" id="<%=options.getOption1()%><%=qid%>"
						name="ans<%=qid%>" value="<%=options.getOption1()%>"> <label
						for="<%=options.getOption1()%><%=qid%>">1. <%=options.getOption1()%></label><br />
					<input type="radio" id="<%=options.getOption2()%><%=qid%>"
						name="ans<%=qid%>" value="<%=options.getOption2()%>"> <label
						for="<%=options.getOption2()%><%=qid%>">2. <%=options.getOption2()%></label><br />
					<input type="radio" id="<%=options.getOption3()%><%=qid%>"
						name="ans<%=qid%>" value="<%=options.getOption3()%>"> <label
						for="<%=options.getOption3()%><%=qid%>">3. <%=options.getOption3()%></label><br />
					<input type="radio" id="<%=options.getOption4()%><%=qid%>"
						name="ans<%=qid%>" value="<%=options.getOption4()%>"> <label
						for="<%=options.getOption4()%><%=qid%>">4.<%=options.getOption4()%></label><br />
					<br /> <br /> <br /> <input type="hidden" name="qtype<%=qid%>"
						value="<%=qued.getQuestion_type()%>"> <input
						type="hidden" name="qmark<%=qid%>"
						value="<%=qued.getQue_marks()%>">
				</div>
			...
			<div class="footer inline-block">
				<input type="submit" class="btn btn-success float-right"
					name="submit" value="Submit"> <a href="../Logout"><input
					type="button" class="btn btn-danger float-left" name="Submit"
					value="LogOut"></a>
				<p>Good Luck...&#128578;</p>
			</div>
	...
	<script>
		...
		function CountDown(duration, display) {
			...
			var interVal = setInterval(function() {
				...
				display.innerHTML = "<b>" + minutes + " m : " + seconds + " s"
						+ "</b>";
				if (timer > 0) {
					--timer;
				} else {
