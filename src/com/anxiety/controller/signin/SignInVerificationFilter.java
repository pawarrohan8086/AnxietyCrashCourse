...
public class SignInVerificationFilter implements Filter {
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		final String validemail = "^[_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@"
				+ "[A-Za-z0-9-]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$";
		final String validpass = "^.*(?=.{6,})(?=..*[0-9])(?=.*[a-z])(?=.*[A-Z]).*$";
		...
		if (request.getParameter("email") != null
			...
			if (request.getParameter("psw").equals(request.getParameter("cpsw"))
				...
				if (request.getParameter("email").matches(validemail)
						&& request.getParameter("psw").matches(validpass)) {
